...
class BreakdownComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    const headerHints = [
      keyHint("tui.editor.cursorLeft", "previous"),
      keyHint("tui.editor.cursorRight", "next"),
      rawKeyHint("1", "7d"),
      rawKeyHint("2", "30d"),
      rawKeyHint("3", "90d"),
    ...
    const header = `${bold("Session breakdown")}  ${tab(7, 0)} ${tab(30, 1)} ${tab(90, 2)}  ${dim(headerHints)}`;
