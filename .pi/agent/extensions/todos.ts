...
class TodoSelectorComponent extends Container implements Focusable {
  ...
  private updateHints(): void {
    const parts = [
      "Type to search",
      keyHint("tui.select.up", "to move up"),
      keyHint("tui.select.down", "to move down"),
      keyHint("tui.select.confirm", "for actions"),
      rawKeyHint("ctrl+x", "to work"),
      rawKeyHint("ctrl+r", "to refine"),
      keyHint("tui.select.cancel", "to close"),
    ];
    this.hintText.setText(this.theme.fg("dim", parts.join(" • ")));
  }
  ...
  constructor(
    ...
    this.addChild(
      new Text(
        theme.fg(
          "dim",
          `${keyHint("tui.select.confirm", "to confirm")} • ${keyHint("tui.select.cancel", "to go back")}`,
        ),
      ),
    );
    this.addChild(new DynamicBorder((s: string) => theme.fg("accent", s)));
...
class TodoDetailOverlayComponent {
  ...
  private buildActionLine(width: number): string {
    const work = this.theme.fg(
      "accent",
      keyHint("tui.select.confirm", "to work on todo"),
    );
    const back = this.theme.fg(
      "dim",
      keyHint("tui.select.cancel", "to go back"),
    );
