...
class QnAComponent implements Component {
  ...
  handleInput(data: string): void {
    ...
    // Plain Enter moves to the next question or shows confirmation on the last question.
    // The newline shortcut still flows through to the editor.
    if (
      this.keybindings.matches(data, "tui.select.confirm") &&
  ...
  render(width: number): string[] {
    ...
    if (this.showingConfirmation) {
      ...
      const confirmMsg =
        `${this.yellow("Submit all answers?")} ` +
        this.dim(
          `(${keyHint("tui.select.confirm", "confirm")}, ${keyHint("tui.select.cancel", "cancel")}, ${rawKeyHint("y", "yes")}, ${rawKeyHint("n", "no")})`,
        );
      lines.push(
      ...
      const controls =
        `${this.dim(keyHint("tui.input.tab", "next"))} · ` +
        `${this.dim(keyHint("tui.select.confirm", "confirm/next"))} · ` +
        `${this.dim(rawKeyHint("shift+tab", "previous"))} · ` +
        `${this.dim(keyHint("tui.input.newLine", "newline"))} · ` +
        `${this.dim(keyHint("tui.select.cancel", "cancel"))}`;
