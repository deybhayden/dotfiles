...
I want you to do a deep dive on PR #$1 using `gh` to pull down a local copy and comparing it to the main branch.
...
"🧙🏻‍♂️ Bendalf:" to make it clear that while my github user is being used, the actual review is being done by Claude.
...
- Preferred: `gh api repos/<owner>/<repo>/pulls/<num>/comments -f body='🧙🏻‍♂️ Bendalf: <comment>' -f commit_id=<sha> -f path=<file> -f position=<pos>`.
...
- Alternate quick note: `gh pr comment <num> --body '🧙🏻‍♂️ Bendalf: ...'` (not line-specific).
...
- Prefix every inline comment with "🧙🏻‍♂️ Bendalf:" to signal Claude-authored feedback.
